# Example 36: Unsynchronized shared counter

The read and write are separate steps, so updates can be lost. A run can still print `6`; neither `yield()` nor tracing guarantees an interleaving. Compare with example35.
...
java -jar target/code-tracer-jar-with-dependencies.jar trace \
  -i examples/example36/Driver.java --multithread --format modern \
