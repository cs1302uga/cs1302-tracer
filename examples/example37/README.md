# Example 37: Wait and notify

The waiter prints `ready`. If it reaches `wait()` before notification, its state appears WAITING. The condition loop also works if notification happens first.
...
java -jar target/code-tracer-jar-with-dependencies.jar trace \
  -i examples/example37/Driver.java --multithread --format modern \
