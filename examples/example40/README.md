# Example 40: Workers outliving main

The non-daemon worker completes even after `main` returns. The sleeping daemon does not keep the JVM alive. Relative output order is not guaranteed by the sleep duration.
...
java -jar target/code-tracer-jar-with-dependencies.jar trace \
  -i examples/example40/Driver.java --multithread --format modern \
