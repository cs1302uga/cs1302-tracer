# Example 41: Executor timeout

The task completes but its executor stays alive. Capture stops with `timeout`, preserves the partial trace, and exits with code 3. A timeout alone does not establish a deadlock.
...
java -jar target/code-tracer-jar-with-dependencies.jar trace \
  -i examples/example41/Driver.java --multithread --format modern \
  --result-envelope --timeout-ms 3000 --max-snapshots 500 \
